...
const TWILIO_API_KEY_SID = Deno.env.get(
  ...
  try {
    ...
    if (!twilioResponse.ok) {
      ...
      console.error(
        ...
        { httpStatus: twilioResponse.status, twilioCode },
      );
    ...
    if (updateError) {
      console.error("Supabase user update failed");
      return jsonResponse(
        { error: "Unable to confirm phone number" },
        500,
      );
    }
